...
class Vl53l0x {
  ...
  public:
    ~Vl53l0x() {}
    VL53L0X_Error begin(uint8_t address, bool initWire);
    ...
    VL53L0X_Error startContinuousRangingMeasurement();
    VL53L0X_Error isReadyContinuousRangingMeasurement(bool &ready);
    VL53L0X_Error readContinuousRangingMeasurement(VL53L0X_RangingMeasurementData_t* RangingMeasurementData);

    VL53L0X_Error changeAddress(uint8_t address);
  private:
    ...
    enum _MeasureStatusType {NO_MEASURE, MEASURE_STARTED, MEASURE_READY};
    _MeasureStatusType _measureStatus;
};
