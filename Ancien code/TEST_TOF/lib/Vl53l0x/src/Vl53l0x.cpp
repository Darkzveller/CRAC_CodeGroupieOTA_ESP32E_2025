...
VL53L0X_Error Vl53l0x::startContinuousRangingMeasurement() {
    VL53L0X_Error status = VL53L0X_ERROR_NONE;
    status = VL53L0X_StartMeasurement(pMyDevice);
    if (VL53L0X_ERROR_NONE == status) {
        _measureStatus = MEASURE_STARTED;
    }
    return status;
}

VL53L0X_Error Vl53l0x::isReadyContinuousRangingMeasurement(bool &ready) {
    VL53L0X_Error status = VL53L0X_ERROR_NONE;
    if (_measureStatus != MEASURE_STARTED) {
    ...
    if (VL53L0X_ERROR_NONE != status) {
        ...
        ready =  (1 == stat);
        if (ready) _measureStatus = MEASURE_READY;
    }
    return status;
}

VL53L0X_Error Vl53l0x::readContinuousRangingMeasurement(VL53L0X_RangingMeasurementData_t* RangingMeasurementData) {
    VL53L0X_Error status = VL53L0X_ERROR_NONE;
    if (_measureStatus != MEASURE_READY) {
